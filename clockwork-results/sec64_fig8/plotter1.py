...
#   data["SLO_multiplier_TS"] = A
  ...
  #   else: label += "_C=0"
  #   print(distribution, rate_fg, num_bg, concurrency)
  ...
  # ax2=ax.twinx()
  ax.set_ylabel("Throughput (reqs/s)", color="black", fontsize=13)
  ...
  for num_bg in num_bgs:
    ...
    x_last = max(x_last, max(X))
    ax.plot(X, Y, label=label, color=colors[colors_idx], linestyle=ls[ls_idx])
...
#   df.to_csv(graphdir + tsvfile, sep="\t", index=False)
  ...
  legend = ax.legend(loc="lower left", title="BC Throughput", ncol=1, \
  ...
  plotfile = "fig8_dist5=" + distribution + ".pdf"
